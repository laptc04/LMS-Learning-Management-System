...
function ClassSettings() {
  ...
  const fetchClassTypes = async () => {
    try {
      const response = await apiInstance.get('api/classtype');
  ...
  const handleDelete = async () => {
    try {
      const response = await apiInstance.delete('api/classtype', {
        data: {
          ids: [deleteId],
        },
      });
          ...
          <tbody>
            {displayedClasses.map((c, index) => (
              <tr key={index} className={index % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                ...
                <td className={`py-3 px-4 italic ${c.status ? 'text-blue-500' : 'text-red-600'}`}>
                ...
                <td className="py-3 px-4 text-right space-x-2">
                  <button className="text-orange-500 hover:text-orange-700 transition">
                    <FaEdit size={18}
                      onClick={() => navigate(`/leadership/setting/class-setting/class-edit/${c.id}`)} />
                  </button>
                  <button
                    className="text-red-500 hover:text-red-700 transition"
                    onClick={() => setDeleteId(c.id)} // `c.id` là id lớp học
                  >
                    <FaTrashAlt size={18} />
                  </button>
                </td>
      ...
      {deleteId && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg" style={{ width: 435 }}>
            <h3 className="text-2xl font-bold text-center">Xóa lớp học</h3>
            <p className="text-base mt-5 mb-10 font-normal">Xác nhận muốn lớp học khóa này? Sau khi xoá sẽ không thể hoàn tác.</p>
