...
export default function ClassUpdate() {
    ...
    useEffect(() => {
        const fetchClassTypeById = async () => {
            try {
                const response = await apiInstance.get(`api/classtype/${id}`); // truyền id vào đây
                setClassTypeDetail(response.data.data);
                console.log("Chi tiết loại lớp học:", response.data.data);
            } catch (error) {
                console.error("Lỗi khi gọi API lấy chi tiết lớp học:", error);
            }
        };
    ...
    const handleSave = async () => {
        try {
            ...
            const response = await apiInstance.put(`api/classtype`, payload);

            console.log('✅ Cập nhật thành công:', response.data);
            showToast(response.data.message || 'Cập nhật thành công!', 'success');
            navigate('/leadership/setting/class-setting/');
        } catch (error) {
