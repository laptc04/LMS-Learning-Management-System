...
export default function Completed() {
    ...
    const fetchClasses = async (page: number) => {
        ...
        try {
            ...
            const completedClasses = apiData.filter((item: ClassItem) => new Date(item.startDate) <= currentDate);

            setTests(completedClasses);
            setTotalPages(Math.ceil(completedClasses.length / pageNumber));
        } catch (error: any) {
